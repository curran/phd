...
\begin{figure}[h]
  \caption{An example visualization dashboard with multiple linked views. Selecting a time on the line chart slices the data displayed on the choropleth map. Zooming and panning on the choropleth map filter the country profiles shown in the line chart.}
  \centering
...
\li \For $str \in myStringArray$
\Do
\li   $log(str)$ \Comment also prints \verb1'foo'1, then prints \verb1'bar'1
\End
